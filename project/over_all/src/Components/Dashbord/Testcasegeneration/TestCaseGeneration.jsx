...
const TestCaseGeneration = () => {
  ...
  const fetchUserStories = async (projectName) => {
    try {
      const response = await fetch(`http://localhost:8000/user-stories/${projectName}`);
      if (!response.ok) {
        throw new Error(`Failed to fetch user stories. Status: ${response.status}`);
      }
      const data = await response.json();
  ...
  const fetchAcceptanceCriteria = async (selectedUserStoryId) => {
    try {
        const response = await fetch('http://localhost:8000/acceptance-criteria', {
            method: 'POST',
            headers: {
 ...
 const handleGenerateTestCases = async () => {
  ...
  return (
    <div className="testcase-generation">
        ...
        >
          ...
          {userStories.length > 0 ? (
            userStories.map((story, index) => {
              ...
              return (
                <option key={index} value={storyId}>{story}</option> // Use the ID as the value
              );
            })
          ) : (
      ...
      <button onClick={() => fetchAcceptanceCriteria(selectedUserStory)} disabled={!selectedUserStory || loading}>
